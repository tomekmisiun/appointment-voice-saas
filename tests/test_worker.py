...
def test_scheduled_maintenance_runs_when_lock_acquired(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.worker.reconcile_stale_notifications",
        lambda db: cleanup_calls.__setitem__(
            "notifications_reconciled", cleanup_calls["notifications_reconciled"] + 1
        ) or 0,
    )
    monkeypatch.setattr(
        "app.worker.reconcile_stale_calendar_events",
        lambda db: cleanup_calls.__setitem__(
            "calendar_events_reconciled", cleanup_calls["calendar_events_reconciled"] + 1
        ) or 0,
    )
    monkeypatch.setattr(
