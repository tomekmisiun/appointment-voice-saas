...
# Inherited Foundation Template Freeze Checklist

This is the historical template freeze checklist for the inherited FastAPI
foundation. It is preserved as reference only. Appointment Voice SaaS is now the
active product repository.
Links in the preserved body below refer to the original foundation-era root
filenames; use this `docs/archive/legacy/` directory for the archived foundation
versions.
...
For step-by-step onboarding, see `docs/archive/legacy/template-onboarding.md` and
`docs/archive/legacy/template-usage.md`.
...
- **AI agent workflow:** `.ai-rules/` binding rules, optional `agents/` personas,
  `.commands/` prompts, `docs/development/workflows/ai-workflows.md` (see `AGENTS.md`).
...
6. **Configure GitHub** — Environments `staging` / `production`; see GitHub
   checklist in `docs/operations/runbooks/production-runtime.md`.
...
Full inventory: `.env.example`, `docs/security/secret-management.md`,
...
1. Choose hosting (K8s, PaaS, VM + Compose) and runtime shape (replicas vs
   in-process workers — see `docs/operations/runbooks/production-runtime.md`).
2. Provision managed PostgreSQL, Redis (HA per `docs/architecture/redis-production-contract.md`),
...
10. Set up backups (`docs/operations/backups/overview.md`) and observability
...
- Default tenant migration backfill remains for upgrade safety; canonical seed is
  `make seed-tenant` (see `docs/architecture/tenant-isolation.md`).
