...
| [ ] | P3-005 | P3 | Add recurring staff blocks. | Lunches/recurring unavailable windows. | Complex recurrence UI. | Recurring blocks apply predictably. | Availability tests. | Daily breaks remain bookable. | partial: model decision resolved in `docs/adr/0003-recurring-staff-blocks.md` (Accepted) — new `RecurringStaffBlock` model that subtracts from generated slots, kept separate from `AvailabilityException` to avoid the stale-replacement-window bug a recurrence rule on that table would introduce; implementation (model/migration/service/API/tests) not yet built |
