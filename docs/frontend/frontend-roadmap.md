...
| Risks | A misconfigured `SESSION_SECRET` in production must fail loudly, not silently fall back — covered by tests with no fallback path in the code. Concurrent refresh attempts (Strict Mode double-effect, multiple tabs) must not corrupt the session — covered by the cookie re-check in `lib/auth/actions.ts`, though the in-memory single-flight de-dup is best-effort only within one server process. |
...
| Scope | `/dashboard/bookings` (list) and `/dashboard/bookings/[bookingId]` (detail); status filter (`BookingStatus`) and staff filter (real staff list), offset pagination; staff/service name resolution via the real staff/services list endpoints (small, fetched-once lookup maps — not N+1 per row); loading/empty/error states; enables the "Bookings" nav entry (currently "Coming soon"). Introduces `@tanstack/react-query` (justified here: this is the first page with real interactive client-side filtering/pagination) and a thin `/api/bookings[...]` Route Handler proxy layer for Client Components to call. |
| Out of scope | Cancellation, rescheduling, date-range filtering (**the backend has no date filter on `GET .../bookings` — do not build one**), customer name/phone display (**no backend endpoint resolves `customer_id` to a name/phone at all — show `Customer #<id>` with a visible note, not a fabricated name**), audit/notification/calendar-sync panels (**no booking-scoped audit query endpoint and no notification/calendar read endpoint exist**), Playwright (still nothing destructive to smoke-test yet, but this is the first reasonable candidate branch if a maintainer wants to add it). |
| Backend dependencies | `GET /api/v1/businesses/{id}/bookings`, `GET .../bookings/{id}`, `GET .../staff`, `GET .../services`. **Known gaps to re-verify before starting** (confirmed during 1a's audit, recorded here so they aren't silently rediscovered): no customer lookup endpoint; no date-range filter; audit log endpoint can't filter by `target_booking_id`; no notification/calendar-sync read endpoint. |
| Acceptance | List and detail both reflect the real, current API contract — no fields shown that the backend doesn't actually return. Unauthorized users (no session) can't reach either route (same layout-level enforcement as 1a). |
| Tests | Loading/empty/error states; list rendering with staff/service name resolution; detail rendering; pagination and filter interaction (React Query). |
| Risks | Easy to accidentally re-introduce the date-filter or customer-name gap from the stale `docs/product/owner-dashboard.md` scope doc instead of the verified current contract — re-check the actual route file, not just that doc, before implementing. |
