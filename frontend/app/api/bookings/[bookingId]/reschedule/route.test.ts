...
function makeRequest(body: unknown, headers: Record<string, string> = {}) {
  return new NextRequest("http://localhost:3000/api/bookings/1/reschedule", {
    method: "POST",
...
describe("POST /api/bookings/[bookingId]/reschedule", () => {
  it("rejects a cross-origin request before touching the backend", async () => {
    ...
    server.use(
      http.post(`${BACKEND_URL}/api/v1/businesses/42/bookings/1/reschedule`, () => {
        backendCalled = true;
    ...
    const response = await POST(
      makeRequest({ new_starts_at: "2026-01-16T10:00:00+00:00" }, { origin: "https://evil.example" }),
      routeContext(),
    );
  ...
  it("returns 403 for a non-admin user, without calling the backend", async () => {
    ...
    const response = await POST(
      makeRequest({ new_starts_at: "2026-01-16T10:00:00+00:00" }),
      routeContext(),
    );
  ...
  it("allows a platform_admin through, mirroring the backend's role hierarchy", async () => {
    ...
    server.use(
      ...
      http.get(`${BACKEND_URL}/api/v1/businesses`, () => HttpResponse.json([business(42)])),
      http.post(`${BACKEND_URL}/api/v1/businesses/42/bookings/1/reschedule`, () =>
        HttpResponse.json({ id: 99, status: "confirmed" }),
      ),
    );
    const { POST } = await import("./route");
  ...
  it("rejects a body missing new_starts_at without calling the backend", async () => {
    seedValidSession();
    let backendCalled = false;
    server.use(
    ...
    const response = await POST(makeRequest({ reason: "no new time given" }), routeContext());

    expect(response.status).toBe(400);
    expect(backendCalled).toBe(false);
  });

  it("on success, returns a booking with a different id than the one in the URL", async () => {
    const accessToken = seedValidSession();
    let receivedAuth: string | null = null;
    let receivedBody: { new_starts_at?: string; reason?: string } | null = null;
    server.use(
      http.get(`${BACKEND_URL}/api/v1/auth/me`, () => HttpResponse.json(meResponse("admin"))),
      http.get(`${BACKEND_URL}/api/v1/businesses`, () => HttpResponse.json([business(42)])),
      http.post(`${BACKEND_URL}/api/v1/businesses/42/bookings/1/reschedule`, async ({ request }) => {
        receivedAuth = request.headers.get("authorization");
        receivedBody = (await request.json()) as { new_starts_at?: string; reason?: string };
        return HttpResponse.json({
          id: 99,
          tenant_id: 1,
          business_id: 42,
          ...
          service_id: 10,
          staff_id: 1,
          starts_at: "2026-01-16T10:00:00+00:00",
          ends_at: "2026-01-16T10:30:00+00:00",
          status: "confirmed",
          source: "api",
          cancel_reason: null,
    ...
    const response = await POST(
      makeRequest({ new_starts_at: "2026-01-16T10:00:00+00:00", reason: "Customer requested" }),
      routeContext("1"),
    );
    const json = await response.json();
    ...
    expect(json.id).not.toBe(1);
    expect(receivedAuth).toBe(`Bearer ${accessToken}`);
    expect(receivedBody).toEqual({
      new_starts_at: "2026-01-16T10:00:00+00:00",
      reason: "Customer requested",
    });
  });

  it("propagates a backend conflict (e.g. slot no longer available) as the same status", async () => {
    seedValidSession();
    server.use(
      http.get(`${BACKEND_URL}/api/v1/auth/me`, () => HttpResponse.json(meResponse("admin"))),
      http.get(`${BACKEND_URL}/api/v1/businesses`, () => HttpResponse.json([business(42)])),
