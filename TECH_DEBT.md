...
| AVS-TD-031 | Keypress handler looks up `VoiceSession` by `session_id` alone, never validating that the `To=` field in the Twilio request matches the business associated with that session. After P4-001, there is no `business_id` in the URL, so the original description (URL-path validation) is obsolete; the residual gap is that a forged keypress with a known `session_id` could drive another business's session. Mitigated by mandatory Twilio signature validation. | Low risk (signature validation already blocks forged requests), but if signature validation is ever misconfigured or bypassed, a caller could drive another business's voice session. | Add a `To`-field/`tenant_id` filter to the `VoiceSession` lookup in the keypress handler: verify `session.business.phone == request_to_field`. | Low | — | XS | Open |
...
| ID | Issue | Risk | Priority | Status |
|----|-------|------|----------|--------|
| ARCH-001 | IVR webhook routes by `business_id` in URL, not by phone number. Real telephony requires routing by the `To` field in Twilio webhook. `app/api/routes/twilio_voice.py`. | Cannot support the real product flow where a customer dials a business phone number. | High | Done (P4-001) — `POST /api/v1/webhooks/twilio/voice` now routes by `To=` field matched against `businesses.phone`; no `business_id` in URL |
| ARCH-002 | `BusinessPhoneNumber` model does not exist on `main`. Developed in backup snapshot branch, never merged. | Was a prerequisite for ARCH-001; ARCH-001 is now done via `business.phone` directly (P4-001). Remaining telephony features (T1–T8: operator API, composite FK isolation, frontend telephony card) still depend on this model. | Medium | Open — TELEPHONY-T1/T2 |
